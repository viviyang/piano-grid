...
export function sourceAssetPath(url:string, relative:string) {
  const base=resolve('docs/content/site-master', ['/chords','/chords/a-minor'].includes(url)?'preserved-chords':'');
  if(!relative.startsWith('assets/')||relative.includes('\\')||relative.includes('..')||relative.includes(':'))throw new Error('Invalid asset path');
  const target=resolve(base,relative);if(!target.startsWith(base+sep))throw new Error('Asset escapes source root');return target;
}
