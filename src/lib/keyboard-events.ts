...
export function emitKeyboardPracticeEvent(name: KeyboardPracticeEventName, properties: Record<string, AnalyticsEventValue> = {}) {
  ...
  window.dispatchEvent(new CustomEvent('pianogrid:keyboard-practice-event', { detail: { name, properties: safe, timestamp: Date.now(), externallySent } }));
}
