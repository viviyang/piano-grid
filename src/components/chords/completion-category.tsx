...
export function CompletionCategoryPage({ model }: { model: ReturnType<typeof getCompletionCategory> }) {
  ...
  return <div className="am-page cc-page"><a className="am-skip" href="#main">Skip to content</a><SiteHeader search={null}/><main id="main" className="pr-container" tabIndex={-1}>
    <header className="am-page-heading"><PageBreadcrumb items={[{ label: 'Chords', href: '/chords' }, { label: heading }]}/><h1>{heading}</h1><p className="am-direct-answer">{model.description}</p><p className="cc-scope">This page contains {model.items.length} reference objects across {model.subtypes.length} named types and twelve roots. It is a declared reference collection, not every possible chord name or voicing.</p></header>
