...
export function SiteFooter({url: _url}:{url:string}) {return <footer className="am-site-footer am-screen"><div className="pr-container am-footer-inner"><SiteBrand className="am-footer-brand"/><span className="am-footer-note">Learn piano, one step at a time.</span><a href="#main">Back to top ↑</a></div><nav className="pr-container am-footer-topics" aria-label="Footer navigation">{SITE_NAVIGATION.map(section=><a href={section.href} key={section.href}>{section.label}</a>)}</nav></footer>;}
