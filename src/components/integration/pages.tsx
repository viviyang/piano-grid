...
export function HomePage(){
  ...
  return <div className="am-page in-page home-page"><a className="am-skip ph-skip" href="#main">Skip to content</a><HomeHeader/><main id="main" tabIndex={-1}>
    <section className="ph-hero" aria-labelledby="home-hero-title"><Image className="ph-hero-image" src="/assets/home/east-lake-piano-hero.png" alt="Hands playing a grand piano beside East Lake" fill priority sizes="100vw"/><div className="ph-hero-scrim" aria-hidden="true"/><div className="pr-container ph-hero-grid"><div className="ph-hero-copy"><p className="ph-eyebrow"><span/>Piano reference &amp; practice tools</p><h1 id="home-hero-title"><span>Clear Piano References. </span><em>Better Practice.</em></h1><p className="ph-hero-description">Find chords, scales, keyboard notes, beginner songs, sheet music, and practical tools—without digging through long lessons.</p><div className="ph-hero-actions"><a className="ph-button ph-button-primary" href={data.secondaryLink.url}>Explore Piano Tools <HomeArrow/></a><a className="ph-button ph-button-secondary" href="/songs">Browse Songs <HomeArrow/></a></div><nav className="ph-quick-links" aria-label="Quick piano tasks">{quickLinks.map((item,index)=><span key={item.url}><a href={item.url}>{item.label}</a>{index<quickLinks.length-1&&<i aria-hidden="true"/>}</span>)}</nav></div></div></section>
