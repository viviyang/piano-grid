...
function PrintableCard({item}:{item:PrintableDestination}){
  return <article className={item.available?'in-print-card':'in-print-card in-unavailable'}><div><span>{item.available?'Available now':'Not yet available'}</span><h3>{item.label}</h3><p>{item.task}</p></div>{item.available?<div className="in-card-actions"><a href={item.url}>Open resource</a>{item.downloadURL&&<a href={item.downloadURL} download>Download PDF</a>}</div>:<p className="in-status">No live link in this preview.</p>}</article>;
}
function HomeFooter(){
  return <footer className="ph-footer"><div className="pr-container ph-footer-main"><a className="ph-footer-brand" href="/"><HomeBrandMark/><span>Piano Reference</span></a><nav aria-label="Footer navigation"><a href="/keyboard-notes">Keyboard Notes</a><a href="/chords">Chords</a><a href="/scales">Scales</a><a href="/songs">Songs</a><a href="/guide">Learn</a><a href="/tools">Tools</a></nav></div><div className="pr-container ph-footer-bottom"><p>Clear references for the moments you sit down to play.</p><a href="#main">Back to top ↑</a></div></footer>;
}
export function HomePage(){
  ...
  return <div className="am-page in-page home-page"><a className="am-skip ph-skip" href="#main">Skip to content</a><HomeHeader/><main id="main" tabIndex={-1}>
    <section className="ph-hero" aria-labelledby="home-hero-title"><Image className="ph-hero-image" src="/assets/home/east-lake-piano-hero.png" alt="Hands playing a grand piano beside East Lake" fill priority sizes="100vw"/><div className="ph-hero-scrim" aria-hidden="true"/><div className="pr-container ph-hero-grid"><div className="ph-hero-copy"><p className="ph-eyebrow"><span/>Piano reference &amp; practice tools</p><h1 id="home-hero-title"><span>Learn the notes. Find the music.</span><em>Practice with purpose.</em></h1><p className="ph-hero-description">Explore piano chords, scales, keyboard notes, and song guides. Understand what you play. Find your next step.</p><div className="ph-hero-actions"><a className="ph-button ph-button-primary" href={data.secondaryLink.url}>Explore Piano Tools <HomeArrow/></a><a className="ph-button ph-button-secondary" href="/songs">Browse Songs <HomeArrow/></a></div><nav className="ph-quick-links" aria-label="Quick piano tasks">{quickLinks.map((item,index)=><span key={item.url}><a href={item.url}>{item.label}</a>{index<quickLinks.length-1&&<i aria-hidden="true"/>}</span>)}</nav></div></div></section>
...
export function ToolsPage(){
  ...
  const availableLookups=data.lookupLinks.filter(item=>item.available),availablePrintables=data.printables.filter(item=>item.available),plannedPrintables=data.printables.filter(item=>!item.available);
  return <Shell model={data.model} current="Tools"><section className="in-tool-directory" aria-labelledby="in-tools-title"><div className="in-section-head"><p>Working references</p><h2 id="in-tools-title">Choose the result you need</h2></div><div className="in-lookup-grid">{availableLookups.map((item,index)=><DestinationCard item={item} index={index} key={item.url}/>)}</div></section>
    <section className="in-printables" aria-labelledby="in-printables-title"><div className="in-section-head"><p>Original downloads</p><h2 id="in-printables-title">Print something useful</h2></div><div className="in-print-grid">{availablePrintables.map(item=><PrintableCard item={item} key={item.url}/>)}</div><details className="in-planned"><summary>{plannedPrintables.length} more printables are not yet available</summary><ul>{plannedPrintables.map(item=><li key={item.url}>{item.label} — {item.task}</li>)}</ul></details></section>
    <Reading blocks={data.model.blocks}>{block=>block.id==='jobs'?<p className="in-release-note">Chord finder is not yet available; use the chord reference for known chord names.</p>:block.id==='printables'?<p className="in-release-note">Only the resources marked available above have live links in this preview.</p>:null}</Reading>
  </Shell>;
}
