...
import '../chords/piano-surface.css';
...
function PianoKeyboard({compact=false,active,sounding,onPlay}:{compact?:boolean;active:number[];sounding:number[];onPlay:(midi:number)=>void}){
  ...
  return <div className="ph-keyboard" role="group" aria-label="Interactive piano keyboard. Use left and right arrow keys to move between notes, and Enter or Space to play.">{whites.map(midi=><button key={midi} type="button" data-midi={midi} tabIndex={midi===(compact?57:48)?0:-1} className={`ph-piano-key ph-white-key ${[57,60,64].includes(midi)?'ph-chord-key':''} ${active.includes(midi)?'ph-note-active':''} ${sounding.includes(midi)?'ph-sounding':''}`} aria-label={`Play ${noteName(midi)}`} onClick={()=>onPlay(midi)} onKeyDown={event=>move(event,midi)}><span className="ph-key-dot" aria-hidden="true">{noteName(midi).replace(/\d+$/,'')}</span><span className="ph-key-name" aria-hidden="true">{noteName(midi)}</span></button>)}{blacks.map(({midi,edge})=><button key={midi} type="button" data-midi={midi} tabIndex={-1} className={`ph-piano-key ph-black-key ${active.includes(midi)?'ph-note-active':''} ${sounding.includes(midi)?'ph-sounding':''}`} style={keyPosition(edge,keys)} aria-label={`Play ${noteName(midi)}`} onClick={()=>onPlay(midi)} onKeyDown={event=>move(event,midi)}><span className="ph-black-key-shine" aria-hidden="true"/></button>)}</div>;
}
