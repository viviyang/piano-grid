...
export function KeyboardDiagram({keys,selected=null,marked=[],sounding=[],keyRoles={},onSelect,octaves=true,keyLabels={},accessibleLabels,label='Piano keyboard',ready=true,showVisualLabels=true,selectedState='selected',selectedLabel,fit=false,autoCenter=true,className='',isKeyDisabled}:{keys:PianoKey[];selected?:number|null;marked?:number[];sounding?:number[];keyRoles?:Partial<Record<number,KeyboardKeyRole>>;onSelect?:(k:PianoKey)=>void;octaves?:boolean;keyLabels?:Record<number,string>;accessibleLabels?:Record<number,string>;label?:string;ready?:boolean;showVisualLabels?:boolean;selectedState?:'selected'|'wrong'|'correct'|'revealed';selectedLabel?:string;fit?:boolean;autoCenter?:boolean;className?:string;isKeyDisabled?:(key:PianoKey)=>boolean}) {
  ...
  return <div ref={ref} className={`kn-key-scroll ${className}`.trim()} role="region" aria-label={label} tabIndex={onSelect?-1:0}>
    <div className="kn-keyboard" style={{width:fit?'100%':`${Math.max(count,1)*5.25}rem`}}>
      {keys.map(k=>{const white=k.color==='white',index=leading+whites.filter(w=>w.midi<k.midi).length,labels=(keyLabels[k.midi]??(octaves?k.label_with_octave:k.default_label)).split(' / '),isMarked=marked.includes(k.midi);
        ...
        const accessibleLabel=accessibleLabels?.[k.midi]??keyLabels[k.midi]??k.label_with_octave;
