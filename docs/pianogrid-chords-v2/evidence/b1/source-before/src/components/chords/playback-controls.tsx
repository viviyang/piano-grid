...
export function PlaybackControls({ready,state,mode,onPlay,onStop}:{ready:boolean;state:AudioStatus;mode:PlaybackMode|null;onPlay:(mode:PlaybackMode)=>void;onStop:()=>void}) {
 return <div className="am-playback-actions"><button type="button" className={cn('am-button am-primary am-play-btn',state==='playing'&&mode==='together'&&'am-is-playing')} disabled={!ready||state==='unavailable'} onClick={()=>onPlay('together')}><Icon name="play"/><span>Play chord</span></button><button type="button" className={cn('am-button am-secondary am-sequence-btn',state==='playing'&&mode==='ascending'&&'am-is-playing')} disabled={!ready||state==='unavailable'} onClick={()=>onPlay('ascending')}><Icon name="sequence"/><span>Play notes one at a time</span></button><button type="button" className="am-button am-secondary am-stop-btn" disabled={!['loading','playing'].includes(state)} onClick={onStop}><Icon name="stop"/><span>Stop</span></button></div>;
}
