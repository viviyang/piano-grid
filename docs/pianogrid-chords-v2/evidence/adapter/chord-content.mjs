import { readAuthorizedPage } from './site-content.mjs';
import { isPublicRoute } from './site-routes.mjs';
const detailCopy = {
...
const empty = (heading) => ({ heading, paragraphs: [], steps: [], table: null, links: [] });
export function getChordDetail(url) {
    const { page, master } = readAuthorizedPage(url);
    if (!['/chords/a-major', '/chords/c-major'].includes(url))
        throw new Error('Wrong detail adapter');
    const copy = detailCopy[url];
    const expected = ['Notes and keyboard position', 'What to notice', 'Compare three positions'];
    if (page.blocks.length !== 3 || page.blocks.some((b, i) => b.heading !== expected[i] || typeof b.body !== 'string'))
        throw new Error(`Unknown/missing core block: ${url}`);
    const prefix = url.split('/').at(-1), d = page.data, contract = master.legacy_chords_support.playback_contract;
    const voicings = d.voicings.map((v) => {
        if (v.notes.length !== 3 || v.midi.length !== 3 || JSON.stringify(v.keyboard_highlights.map(h => h.midi)) !== JSON.stringify(v.midi))
            throw new Error(`Missing voicing: ${url}`);
        ...
        const events = (midis, mode) => midis.map((midi, i) => ({ midi, frequency_hz: 440 * 2 ** ((midi - 69) / 12), onset_ms: mode === 'together' ? 0 : contract.ascending_onsets_ms[i], duration_ms: mode === 'together' ? contract.together_duration_ms : contract.ascending_duration_ms }));
    ...
    const data = { url, namespace: prefix, toolId: `${prefix}-result`, rangeLabel: 'C4–C6', pdf: { url: `/reference/assets/chord-${prefix}.pdf`, label: `Download ${name} PDF` }, defaultId: `${prefix}--${d.default_voicing}`, options: voicings.map(v => ({ value: v.voicing_id, label: v.inversion_label })), chord: { name_en: name, symbol: d.symbol, root_spelling: d.root, note_spellings: d.pitch_classes, formula_degrees: ['1', '3', '5'] }, voicings, whitePitchClasses: master.legacy_chords_support.shared_data.conventions.white_pitch_classes, microcopy: master.pages['/chords/a-minor'].microcopy, heading: copy.heading, toolHeading: page.blocks[0].heading, printDisclaimer: 'The marked keys show pitches, not a hand shape or a required fingering.' };
    const blocks = [{ block_id: `${prefix}-intro`, content: { ...empty(copy.heading), paragraphs: [page.blocks[0].body] } }, { block_id: data.toolId, content: empty(page.blocks[0].heading) }, { block_id: `${prefix}-notice`, content: { ...empty(page.blocks[1].heading), paragraphs: [page.blocks[1].body] } }, { block_id: `${prefix}-inversions`, content: { ...empty(page.blocks[2].heading), paragraphs: [page.blocks[2].body], table: { columns: ['Position', 'Symbol', 'Notes, low to high', 'Bass'], rows: voicings.map(v => [v.inversion_label, v.chord_symbol, v.notes_low_to_high.map(n => n.display_pitch).join('–'), v.bass_spelling]) } } }, { block_id: `${prefix}-reference`, content: { ...empty('Chord reference'), paragraphs: [`Common names: ${d.aliases.join(', ')}.`, `Intervals from the root: ${d.intervals.join(', ')}.`] } }, { block_id: `${prefix}-print`, content: empty('Print this chord reference') }];
    // Independently sourced RH example remains separate from all null inversion fingerings.
    if (d.verified_fingering_example) {
        const f = d.verified_fingering_example;
        blocks.splice(4, 0, { block_id: `${prefix}-fingering-example`, content: { ...empty('Right-hand root-position example'), paragraphs: [`${f.scope}: ${f.notes.join('–')} → ${f.fingers.join('–')}.`] } });
    }
    const related = d.related.filter((u) => isPublicRoute(u));
    if (related.length)
        blocks.push({ block_id: `${prefix}-related`, content: { ...empty('Related references'), links: related.map((u) => ({ url: u, label: u === '/chords' ? 'Piano chord chart' : u, published: true })) } });
    const byId = Object.fromEntries(blocks.map(b => [b.block_id, b]));
    return { data, blocks, byId, metadata: { ...page.metadata, title: copy.title, description: copy.description }, answer: page.blocks[0].body, introduction: [], searchSections: blocks.filter(b => b.block_id !== `${prefix}-intro`).map(b => ({ id: b.block_id, heading: b.content.heading, text: JSON.stringify(b.content) })) };
}
const pitchClasses = { C: 0, D: 2, E: 4, F: 5, G: 7, A: 9, B: 11 };
const displayAccidentals = (value) => value.replaceAll('#', '♯').replaceAll('b', '♭');
const rootName = (root) => root.replace('#', '-sharp').replace('b', '-flat');
const chordID = (root, quality) => `${rootName(root).toLowerCase()}-${quality}`;
function parsedPitch(value) { const match = /^([A-G])([#b]?)(-?\d+)$/.exec(value); if (!match)
    throw new Error(`Invalid pitch: ${value}`); const [, letter, accidental, octaveText] = match, octave = Number(octaveText), offset = accidental === '#' ? 1 : accidental === 'b' ? -1 : 0; return { letter, accidental, octave, midi: (octave + 1) * 12 + pitchClasses[letter] + offset }; }
function keyChordItem(row) {
    if (!['major', 'minor'].includes(row.quality) || row.notes.length !== 3 || row.reference_voicing.length !== 3 || row.fingering !== null)
        throw new Error(`Unsupported key-chord row: ${row.symbol}`);
    ...
    if (JSON.stringify(row.semitones_from_root) !== JSON.stringify(expected) || JSON.stringify(intervals) !== JSON.stringify(expected) || JSON.stringify(parsed.map(note => `${note.letter}${note.accidental}`)) !== JSON.stringify(row.notes))
        throw new Error(`Invalid key-chord spelling: ${row.symbol}`);
    ...
    if (notes.some(note => note.midi < 48 || note.midi > 72))
        throw new Error(`Key-chord voicing outside C3-C5: ${row.symbol}`);
    const root = row.notes[0], id = chordID(root, row.quality), name = `${rootName(root)} ${row.quality}`, midis = notes.map(note => note.midi), events = (mode) => midis.map((midi, index) => ({ midi, frequency_hz: 440 * 2 ** ((midi - 69) / 12), onset_ms: mode === 'together' ? 0 : index * 600, duration_ms: mode === 'together' ? 1200 : 500 }));
...
export function getChordCenter() {
    ...
    if (JSON.stringify(page.blocks.map((b) => b.block_id)) !== JSON.stringify(ids))
        throw new Error('Unknown/missing center block');
    const originalItems = page.data.chord_ids.map((id) => { const c = shared.chords[id], v = shared.voicings[c.root_voicing_id]; if (!v)
        throw new Error(`Missing ${id}`); return { id, tones: c.note_spellings, formula: c.formula_degrees, name: c.name_en, root: c.root_spelling, quality: c.quality, url: isPublicRoute(c.canonical_url) ? c.canonical_url : null, voicing: { voicing_id: v.voicing_id, inversion_label: v.inversion_label, chord_symbol: v.chord_symbol, bass_spelling: v.bass_spelling, notes_low_to_high: v.notes_low_to_high.map((n) => ({ display_pitch: n.display_pitch, midi: n.midi })), diagram: v.diagram, playback: { together: v.playback.together, ascending: v.playback.ascending }, print_data: v.print_data } }; });
    const keyPage = master.pages['/chords/by-key'];
    if (!keyPage || keyPage.data.keys.some((key) => !key.evidence_status.includes('formula crosscheck passed')))
        throw new Error('Key-chord source rows are not ready for reuse');
    const keyRows = new Map();
    for (const key of keyPage.data.keys)
        for (const row of key.chords) {
            if (!['major', 'minor'].includes(row.quality))
                continue;
            const identity = `${row.notes[0]}|${row.quality}`, prior = keyRows.get(identity);
            if (prior && JSON.stringify([prior.notes, prior.reference_voicing]) !== JSON.stringify([row.notes, row.reference_voicing]))
                throw new Error(`Conflicting key-chord rows: ${identity}`);
            keyRows.set(identity, row);
        }
    const itemMap = new Map(originalItems.map(item => [item.id, item]));
    for (const row of keyRows.values()) {
        const item = keyChordItem(row);
    ...
    const items = order.map(id => itemMap.get(id));
    if (items.some(item => !item) || items.length !== itemMap.size)
        throw new Error('Expanded chord collection order is incomplete');
    const completeItems = items;
    const comparisons = Object.values(shared.comparisons).map(pair => { const p = pair; return { left: completeItems.find(i => i.voicing.voicing_id === p.left), right: completeItems.find(i => i.voicing.voicing_id === p.right) }; });
    const blocks = page.blocks.map((block) => ({ block_id: block.block_id, content: { ...block.content, paragraphs: [...block.content.paragraphs], steps: [...block.content.steps], links: [...block.content.links], table: block.content.table ? { columns: [...block.content.table.columns], rows: block.content.table.rows.map(row => [...row]) } : null } }));
    ...
    const faq = blocks.find(block => block.block_id === 'chords-questions').content.table;
    faq.rows = faq.rows.map(row => row[0] === 'Are these all the chords on piano?' ? [row[0], 'No. This chart covers 19 major and minor triads. Diminished, seventh, power, and jazz chords are not included in this collection.'] : row);
