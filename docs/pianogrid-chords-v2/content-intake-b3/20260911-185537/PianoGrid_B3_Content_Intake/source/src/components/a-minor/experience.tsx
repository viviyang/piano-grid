  ...
  return <PrintContext.Provider value={{ready,print,pdf:data.pdf}}><SelectedVoicingContext.Provider value={selectedVoicingId}><div className="am-page" data-selected-voicing={selectedVoicingId} data-position={voicing.inversion_label}>
    ...
    <main className="pr-container am-screen" id="main">{heading}
      <section className="am-tool" id={data.toolId} data-block-id={data.toolId} data-voicing-id={selectedVoicingId} data-audio-state={audio.state} tabIndex={-1} aria-labelledby="tool-heading">
        ...
        <dl className="am-quick-facts" aria-label="Quick facts"><div><dt><span className="am-fact-icon"><ChordTopicIcon id={`${data.namespace}-intro`}/></span>Notes</dt><dd>{data.chord.note_spellings.join(' · ')}</dd></div><div><dt><span className="am-fact-icon"><ChordTopicIcon id={`${data.namespace}-${data.chord.quality}`}/></span>Quality</dt><dd>{data.chord.quality[0].toUpperCase()+data.chord.quality.slice(1)} triad</dd></div><div><dt><span className="am-fact-icon"><ChordTopicIcon id={`${data.namespace}-inversions`}/></span>Formula</dt><dd>{data.chord.formula_degrees.join(' · ').replace('b','♭')}</dd></div><div><dt><span className="am-fact-icon"><ChordTopicIcon id={data.toolId}/></span>Keyboard range</dt><dd>{data.rangeLabel}</dd></div></dl>
