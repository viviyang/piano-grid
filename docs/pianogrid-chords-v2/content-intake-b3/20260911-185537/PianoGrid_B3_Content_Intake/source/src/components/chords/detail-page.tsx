...
export function ChordDetailPage({model}:{model:ChordDetailModel}) {
  ...
  const heading=<><header key="heading" className="am-page-heading" data-block-id={`${prefix}-intro`}><PageBreadcrumb items={[{ label: 'Chords', href: '/chords' }, { label: data.chord.name_en }]}/><h1><ChordSectionTitle id={`${prefix}-intro`} text={data.heading}/></h1><p className="am-direct-answer">{answer}</p></header><ChordPageToc items={model.tocItems}/></>;
  ...
  return <ChordDetailExperience data={data} heading={heading} toolNotes={byId[data.toolId].content.paragraphs.map(p=><p key={p}>{p}</p>)} introduction={intro} searchSections={searchSections}>
    {blocks.filter(b=>![`${prefix}-intro`,data.toolId].includes(b.block_id)).map(block=>{const {block_id:id,content:c}=block;
      if(id===`${prefix}-fingering-example`)return <FingeringGuide key={id} block={block} examples={model.fingeringExamples} sources={model.sources} defaultVoicingId={data.defaultId}/>;
