...
try{
 ...
 await root.selectOption('Cb');check('Hub no-match state',await hub.locator('.ch-empty').count()===1);await hub.getByRole('button',{name:'Show the full collection'}).click();check('Hub clear restores 19',await hub.locator('.ch-result').count()===19);
