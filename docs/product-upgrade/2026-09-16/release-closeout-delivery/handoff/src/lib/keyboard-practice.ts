...
/** Shared practice deep-link: hash-only entry stays on /keyboard-notes (RC02). */
export const PRACTICE_ENTRY_HASH = '#note-trainer';

export function isPracticeEntryHash(hash: string) {
  return hash === PRACTICE_ENTRY_HASH || hash === 'note-trainer';
}

export function practiceEntryHref(params?: URLSearchParams | string) {
  const query = typeof params === 'string'
    ? (params ? `?${params.replace(/^\?/, '')}` : '')
    : params && params.size
      ? `?${params}`
      : '';
  return `/keyboard-notes${query}${PRACTICE_ENTRY_HASH}`;
}
