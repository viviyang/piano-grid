...
async function runDevice(browser, { width, height, device, states }) {
  ...
  check(`${device} landing hides third spoiler`, !/raising only the third|A–C–E|A–C♯–E/i.test((await page.locator('.hd-challenge').innerText()) + '\n' + (await page.locator('.hd-more').innerText())));
  ...
  if (states.includes('audio-error')) {
    ...
    check(`${device} real audio-error path`, await page.locator('.hd-audio-error').isVisible());
    ...
    await page.waitForFunction(() => !document.querySelector('.hd-audio-error'), null, { timeout: 8000 });
    check(`${device} retry clears audio-error`, await page.locator('.hd-audio-error').count() === 0);
  }
