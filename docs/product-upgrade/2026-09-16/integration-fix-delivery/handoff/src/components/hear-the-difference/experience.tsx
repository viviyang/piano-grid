...
const AUDIO_COPY = {
  ...
  audio_error: 'Audio isn’t available right now. Try again or continue with the chord references below.',
  audio_unavailable: 'Audio isn’t available right now. Try again or continue with the chord references below.',
} as const;
...
export function HearTheDifferenceExperience({
  ...
  async function runComparison(kind: 'start' | 'replay' = 'start') {
    ...
    setGuess(null);
    if (phase === 'reveal') {
      // Keep reveal when replaying after reveal is not expected; comparisons before reveal reset feedback.
    } else if (phase !== 'wrong' && phase !== 'hint') {
      setPhase('landing');
    }
    emitHearEvent(kind === 'replay' ? 'one_note_replay' : 'one_note_compare_start', { pair: pairId, source: source ?? null });
    const ok = await audio.playComparison(pair);
    setBusy(false);
    if (!ok) {
      if (audio.state === 'error' || audio.state === 'unavailable' || document.hidden) {
        if (!document.hidden) {
          setAudioError(true);
          emitHearEvent('one_note_audio_error', { pair: pairId, phase: audio.phase });
        }
      }
      return;
    }
    emitHearEvent('one_note_compare_complete', { pair: pairId });
    if (phase === 'reveal') return;
    setPhase(current => (current === 'wrong' || current === 'hint' ? current : 'ready'));
  }

  async function playSide(side: 'minor' | 'major') {
    if (!comparisonReady || busy) return;
    setBusy(true);
    emitHearEvent('one_note_single_replay', { pair: pairId, side });
    await audio.playSide(pair, side);
    setBusy(false);
  }
      ...
      <section className="hd-challenge" id="challenge" ref={challengeRef} aria-labelledby="hd-challenge-title">
        <div className="hd-challenge-head">
          <div>
            ...
            <p className="hd-muted">Start with the sound. The chord names and notes appear after you answer.</p>
          </div>
        ...
        {audioError && (
          <div className="hd-audio-error" role="alert">
            <strong>Audio isn’t available right now.</strong>
            <p>Try again or continue with the chord references below.</p>
        ...
        <div className="hd-guess">
          ...
          {phase === 'reveal' && revealMethod === 'correct' && (
            <div className="hd-feedback info" role="status"><strong>Yes.</strong> You heard the changed voice.</div>
          )}
          ...
          {(phase === 'wrong' || phase === 'hint') && (
            <div className="hd-wrong-actions">
              ...
              <button type="button" className="am-button am-secondary" onClick={showHint}>Hint</button>
              <button type="button" className="am-button am-tertiary" onClick={showAnswer}>Show answer</button>
            </div>
          )}
        </div>
        ...
        {phase === 'reveal' && (
          <section className="hd-reveal" ref={revealRef} aria-labelledby="hd-reveal-title">
            <div className="hd-reveal-grid">
              <div>
                <p className="hd-eyebrow">{revealMethod === 'show_answer' ? 'Answer revealed' : helpUsed ? 'Comparison explained' : 'You found it'}</p>
        ...
        <details
          className="hd-learn"
          onToggle={event => {
            if ((event.currentTarget as HTMLDetailsElement).open) setHelpUsed(true);
          }}
        >
          <summary>Learn how the comparison works</summary>
          ...
          <p className="hd-muted">The answer is still yours to choose above. Opening this section marks the round as completed with help.</p>
        </details>
      </section>
