# READY_FOR_MANUAL_ACCEPTANCE，scope=P1

## 2026-09-22 已批准阻塞修复后的当前结论

[已核实] P1 实现及本轮获批的音频启动合同、两份既有 PDF 修复已完成本地验证；九页桌面/移动衔接区共 18 张截图已补齐。本状态表示可以进入人工验收，不表示发布就绪或真人听音已通过。下方旧结论保留为历史记录；完整修复前报告另存 `checks/product-system-p1-2026-09-22/remediation/RESULT-before-remediation.md`。

[已核实] 本轮仅操作 `piano-main-product-audit` 的 `codex/product-system-p1`。阶段开始和交付前 fetch 的 origin/main 与 HEAD 均为 `d15c0142cc6d00b01d060ba17092000a5bf060ca`，无需合并。P0/P1 累积未提交实现保留；没有 commit、push、merge、部署或后台统计测试。原工作区仅做只读状态核对。

[已核实] 原工作区 status 与开始时并非完全相同：新增未跟踪的 `docs/product/feedback-launch-gates.md`；本轮未创建、读取或修改该文件。`final-scope.json` 的 `originalWorkspaceStatusEqual=false` 如实保留，未将外部变化抹除。

### 本轮修复及实际证据

以下相对证据均位于 `checks/product-system-p1-2026-09-22/remediation/`。

| 项目 | 当前实现与验证 | 证据 |
|---|---|---|
| 音阶 preparing 与音频错位 | ReferenceAudio 新增启动回调，返回实际调度起点；原 play Promise 仍在播放结束时完成。音阶据启动回调开始倒计时；启动取消、失败及播放中失败会终止相应引导 | `audio-unit-final.log`：9/9；`chrome-audio-final.json`；`chrome-regression.json` |
| 音符高亮停滞 | 用可取消的 16ms timer 读取 AudioContext 时钟，避免依赖被遮挡窗口的动画帧。未改音符、顺序、时长或波形 | `build-clock.log`；`chrome-audio-analysis.json`；原失败采样保留 |
| 共享音频回归 | Chrome 最终生产服务上五个和弦页均观察到 playing，点击各页实际停止控件后为 stopped 或 idle | `chrome-shared-audio-assessment.json`，原始与纠正定位记录均保留 |
| Letter/A4 PDF | 只重生成原来的两份三页资产。修正生成器 paper globals 作用域与谱号标签间距；两份 PDF 精确标记 binary。六页全部渲染并实际查看 | `pdf-validation.json`：严格解析、纸张、音名/指法、每行五黑键、封面间距及文本边界 PASS；`letter-1..3.png`、`a4-1..3.png` |
| 九页视觉与布局 | Chrome 补齐 1440/390 截图；最终 4354 服务另跑九页 × 390/768/1440，27/27 单 H1、单衔接区、单主动作、最多三链接、无页面横溢出 | `screenshot-manifest.json`、`chrome-layout-final.json` |
| 规则与回归 | npm check、data、scale、来源适配检查通过；状态/测量/hash 14 项回归通过；最终生产构建 exit 0 | `check-clock.log`、`data-final.log`、`scale-final.log`、`adapter.log`、`regression-unit.log`、`build-clock.log` |
| 范围保护 | 207 HTML metadata/H1 无差异，静态/动态路由集合相同；审计与锁文件保持；public 只变两份获批 PDF | `metadata-comparison.json`、`final-scope.json` |

### 同步验证的精确边界

[已核实] 最终 Chrome 连续采样 178 次，92 次具有发声音符标记，八个音均出现，练习从 counting 到 playing 再到 completed，自然结束清空高亮。暂停/继续/重置、设置变更取消、静音引导通过。91/92 发声样本中音名提示与高亮一致；一次切换边界仍显示 E4 提示、F4 高亮，36ms 后的下一次采样已一致。相邻样本时间跨度为 91ms，不能当作实际偏差持续时间。

`chrome-audio-analysis.json` 中额外的 80ms 上界探测为 **false，保留不改**：离散采样不足以证明该上限。这里没有把它改成 PASS，也没有把浏览器 DOM 状态当作扬声器实听证据。原先声音播放而引导仍 preparing、长时间不更新高亮的问题在最终样本中未再出现；严格时序与实际听感仍须人工验收。本轮不承诺零延迟。

### 失败记录、复用与未验项

- Tabbit 的前台限制与截图超时记录保留；用户提出 Chrome 后使用已安装 Chrome 工具继续，没有改全局配置或关闭权限。Chrome 超长页面整页截图曾超时，改为实际衔接区域截图；/chords 截图使用现有 C 根音过滤。18 张截图不是全站全页面视觉审计。
- 第一轮共享音频脚本误定位其他键盘的 Stop sound；纠正为实际页面 Stop 后验证停止。脚本最初只接受 idle，而合法 stopped 状态也会出现；原 false 保留，单独 assessment 明确修正依据。
- 18 张截图来自 4353/4354 两次生产构建，二者 UI 相同，仅后者修改音频时钟轮询；最终 27 项布局检查及五和弦音频回归在 4354 完成。未重复无变化的全部 P1 接收/测量流程，复用下面历史报告列出的已有结果。
- `browser-audio-clock.js`、`browser-regression.js`、`browser-nine.js` 是中途准备的脚本，不代表全部已执行；以实际 JSON/log receipt 为准。旧全套浏览器集成脚本、Lighthouse、完整无 JS 流程未补跑。
- **BACKEND_NOT_VERIFIED**：真实 analytics 后台收数、事件归因与分母；没有发送生产测试事件。
- **MANUAL_PENDING**：真人听音与音画同步、iOS/Android 真机、读屏、实体打印、具名音乐专业审阅、PDF 无障碍决定。PDF 本地渲染通过不等于无障碍签核。
- 不进入 P2，不新增 URL，不改变索引策略、TDH、导航、主题或依赖。P0 历史 RESULT 不回写为当时已通过。

### 本轮新增/修改文件

本轮音频涉及 `src/lib/a-minor-audio.ts`、`src/components/scales/use-scale-audio.ts`、`src/components/scales/scale-learning.tsx`；PDF 涉及两个既有生成脚本、两份既有 PDF 及 `.gitattributes` 两条精确 binary 规则。新增测试 `scripts/check-audio-start-contract.test.mjs`、`scripts/check-scale-audio-start.test.mjs`、`scripts/check-c-major-starter-pdfs.py`。未批量更新 expected hashes。build 自动改动的 next-env/tsconfig 已恢复阶段开始内容。

---

## 以下为修复前的历史报告（BLOCKED，scope=P1；已由上文当前结论取代）
