...
> **2026-09-10 当前状态覆盖说明：** 本节以下较早的“执行 08／尚未部署／保持 noindex”流程已完成，不再代表当前检查点。08 已独立验收为 `PASS_WITH_NOTES`；首轮 17 条路由已在 `https://pianogrid.com` 的 Vercel 生产环境公开，当前输出 `index,follow`，robots 和 sitemap 已上线。历史执行细则继续作为证据与边界保留。

**现在的唯一下一步：对最终公开构建做独立复验。** 独立复验之后，按真实证据记录具名专业审阅、真机、读屏、打印、听音、PDF 无障碍决定和外链人工检查。未完成项不得写成通过；不自动进入 91 条后做页面。
...
[历史 08 执行约束] 当时的生产配置保持不公开／noindex；该约束已被后续经用户确认的 public-launch 阶段取代。当前生产站点已经允许索引，任何再次改变索引策略都需要新的明确授权。
