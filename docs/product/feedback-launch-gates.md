...
状态：路径级 Vercel 限流规则已生效并完成匿名请求拦截验证；GitHub 最小权限令牌及三项 Preview 环境变量已配置。令牌值曾出现在本次浏览器自动化工具结果中，须在使用前撤销并轮换。新的 Preview 构建受 Hobby 每日部署额度限制，尚未验证真实云端提交；生产反馈功能仍未部署或启用。
...
1. [已完成初次配置，使用前须轮换] GitHub 细粒度令牌 `PianoGrid feedback (Vercel)` 仅选择 `viviyang/pianogrid-feedback`，仓库权限为 **Issues: Read and write**（GitHub 自动附加必需的 Metadata: Read-only），无账户权限；初始有效期至 2026-12-21。令牌已由账户持有人直接保存为 Vercel **Preview Secret** `FEEDBACK_GITHUB_TOKEN`，未写入代码或 Git。但 GitHub 一次性展示的令牌值随后被浏览器自动化工具读入本次任务记录；验收前须由账户持有人撤销该令牌、创建相同范围的新令牌并替换 Preview Secret。新令牌到期前还须轮换；不要使用本地开发测试用的广权限凭据。
...
[已核实] `FEEDBACK_GITHUB_REPOSITORY=viviyang/pianogrid-feedback` 与 `FEEDBACK_ENABLED=true` 已作为 Vercel **Preview Config** 保存；三项新变量均只显示 Preview 作用范围。生产没有配置这些变量，代码的默认开关为关闭。环境变量变更需要新部署才能生效。轮换时必须覆盖现有 Preview Secret，不能把新令牌发到聊天或保存在仓库。
