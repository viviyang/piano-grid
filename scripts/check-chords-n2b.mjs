...
const correctedThreeNoteDescription=value=>value.replace('See keyboard positions, three inversions, sound and a printable reference.','See three keyboard positions, sound and a printable reference.');
...
try{
  ...
  for(const raw of details){
    ...
    check(`${raw.url} static HTTP/metadata`,response.status()===200&&html.includes('<h1')&&!html.includes('BAILOUT_TO_CLIENT_SIDE_RENDERING')&&await nojs.title()===seoRow.title&&await nojs.locator('meta[name=description]').getAttribute('content')===correctedThreeNoteDescription(seoRow.description)&&clean(await nojs.locator('h1').innerText())===seoRow.h1&&await nojs.locator('link[rel=canonical]').getAttribute('href')===`https://pianogrid.com${raw.url}`);
  ...
  const chordNav=hub.locator('.site-nav-group').filter({has:hub.locator('.site-nav-parent-link[href="/chords"]')});await chordNav.hover();check('Desktop navigation has four planned groups',same(await chordNav.locator('.site-nav-link-group h3').allTextContents(),['Browse','More Chords','Explore','Learn']));check('More Chords keeps N2B/N2C/N2D links and adds completion categories',same(await chordNav.locator('.site-nav-link-group').filter({hasText:'More Chords'}).locator('.site-nav-child-link').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href'))),['/chords/diminished','/chords/augmented','/chords/suspended','/chords/seventh','/chords/add','/chords/extended','/chords/altered']));
  const mobile=hub.locator('.site-mobile-section').filter({has:hub.locator('.site-mobile-parent[href="/chords"]')});const mobileLinks=[await mobile.locator('.site-mobile-parent').getAttribute('href'),...await mobile.locator('.site-mobile-children a').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href')))];check('Mobile chord navigation preserves earlier order and appends completion categories',same(mobileLinks,['/chords','/chords/major','/chords/minor','/chords/diminished','/chords/augmented','/chords/suspended','/chords/seventh','/chords/add','/chords/extended','/chords/altered','/chords/by-key','/chord-progressions','/chords/finder','/guide/piano-chords','/keyboard-notes/finger-numbers']));
  const sitemap=await (await hub.request.get(base+'/sitemap.xml')).text(),sitemapPaths=[...sitemap.matchAll(/<loc>https:\/\/pianogrid\.com([^<]*)<\/loc>/g)].map(match=>match[1]||'/');check('Sitemap has 173 unique public URLs while retaining N2B',sitemapPaths.length===173&&new Set(sitemapPaths).size===173&&n2bURLs.every(url=>sitemapPaths.includes(url)),sitemapPaths.length);
  for(const url of ['/chords/extended','/chords/altered'])check(`${url} is published by the completion batch`,(await hub.request.get(base+url)).status()===200&&sitemapPaths.includes(url));
...
const report={executedAt:new Date().toISOString(),base,passed:results.filter(item=>item.passed).length,failed:results.filter(item=>!item.passed).length,counts:{categories:3,details:48,newURLs:51,sitemap:173},manualChecks:['real mobile/tablet touch','screen reader','human listening','physical printing','PDF tag accessibility','independent piano-teacher review'],results};
