...
try{
  ...
  const chordNav=hub.locator('.site-nav-group').filter({has:hub.locator('.site-nav-parent-link[href="/chords"]')});await chordNav.hover();check('Desktop navigation has four planned groups',same(await chordNav.locator('.site-nav-link-group h3').allTextContents(),['Browse','More Chords','Explore','Learn']));check('More Chords exact links after N2D',same(await chordNav.locator('.site-nav-link-group').filter({hasText:'More Chords'}).locator('.site-nav-child-link').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href'))),['/chords/diminished','/chords/augmented','/chords/suspended','/chords/seventh','/chords/add']));
  const mobile=hub.locator('.site-mobile-section').filter({has:hub.locator('.site-mobile-parent[href="/chords"]')});const mobileLinks=[await mobile.locator('.site-mobile-parent').getAttribute('href'),...await mobile.locator('.site-mobile-children a').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href')))];check('Mobile chord navigation order after N2D',same(mobileLinks,['/chords','/chords/major','/chords/minor','/chords/diminished','/chords/augmented','/chords/suspended','/chords/seventh','/chords/add','/chords/by-key','/chord-progressions','/chords/finder','/guide/piano-chords','/keyboard-notes/finger-numbers']));
  const sitemap=await (await hub.request.get(base+'/sitemap.xml')).text(),sitemapPaths=[...sitemap.matchAll(/<loc>https:\/\/pianogrid\.com([^<]*)<\/loc>/g)].map(match=>match[1]||'/');check('Sitemap has 171 unique public URLs while retaining N2B',sitemapPaths.length===171&&new Set(sitemapPaths).size===171&&n2bURLs.every(url=>sitemapPaths.includes(url)),sitemapPaths.length);
  for(const url of ['/chords/extended','/chords/altered'])check(`${url} remains deferred`,(await hub.request.get(base+url)).status()===404&&!sitemapPaths.includes(url));
...
const report={executedAt:new Date().toISOString(),base,passed:results.filter(item=>item.passed).length,failed:results.filter(item=>!item.passed).length,counts:{categories:3,details:48,newURLs:51,sitemap:171},manualChecks:['real mobile/tablet touch','screen reader','human listening','physical printing','PDF tag accessibility','independent piano-teacher review'],results};
