...
try{
  ...
  const chordNav=hub.locator('.site-nav-group').filter({has:hub.locator('.site-nav-parent-link[href="/chords"]')});await chordNav.locator('.site-nav-parent-link').focus();check('Desktop navigation keeps Browse Explore Learn groups',same(await chordNav.locator('.site-nav-link-group h3').allTextContents(),['Browse','Explore','Learn']));check('More Chords keeps exact published family categories',same(await chordNav.locator('.site-nav-third-panel[data-kind="more"] .site-nav-third-link').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href'))),['/chords/seventh','/chords/diminished','/chords/augmented','/chords/suspended','/chords/add','/chords/extended','/chords/altered']));
  const mobile=hub.locator('.site-mobile-section').filter({has:hub.locator('.site-mobile-parent[href="/chords"]')});check('Mobile chord navigation uses two root accordions and a family group',await mobile.locator('.site-mobile-accordion-trigger').count()===3&&await mobile.locator('.site-mobile-accordion-panel[data-kind="major"] a').count()===13&&await mobile.locator('.site-mobile-accordion-panel[data-kind="minor"] a').count()===13&&await mobile.locator('.site-mobile-accordion-panel[data-kind="more"] a').count()===7&&await mobile.locator('a[href="/chords/more"]').count()===0);
