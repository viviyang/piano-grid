...
const selectedLinkIds=new Set(['L003','L005','L006','L007','L008','L011','L012','L013','L014','L015','L018','L019','L020','L022','L023','L024','L027','L030','L031','L032','L035','L036','L037','L038','L039','L066','L067','L068']);
const allPublic=new Set(['/','/tools','/chords','/chords/a-minor','/chords/a-major','/chords/c-major',...routes,'/chords/by-key','/chord-progressions','/keyboard-notes','/keyboard-notes/labeled','/keyboard-notes/chart','/keyboard-notes/finger-numbers','/scales','/scales/c-major','/scales/a-minor','/songs','/songs/easy','/guide','/guide/read-sheet-music','/guide/piano-chords','/tools/blank-sheet-music']);
...
try{
 ...
 const hub=await newPage({javaScriptEnabled:false}),hubResponse=await hub.goto(base+'/chords');check('Hub keeps 25 objects and links every detail',hubResponse.status()===200&&await hub.locator('.ch-result').count()===25&&await hub.locator('.ch-result a[href^="/chords/"]').count()===25);check('Hub links the published C-flat detail',await hub.locator('[data-chord-id="c-flat-major"] a[href="/chords/c-flat-major"]').count()===1);await hub.close();
 const request=await browser.newPage();const sitemap=await request.request.get(base+'/sitemap.xml'),xml=await sitemap.text();check('Sitemap contains the authorized chord and support URLs',routes.every(route=>xml.includes(`https://pianogrid.com${route}</loc>`))&&['/guide/piano-chords','/keyboard-notes/finger-numbers','/chords/by-key','/chords/finder','/chords/c-flat-major'].every(route=>xml.includes(`https://pianogrid.com${route}</loc>`)),xml.match(/<loc>/g)?.length);
 for(const route of ['/guide/piano-chords','/keyboard-notes/finger-numbers']){const response=await request.request.get(base+route);check(`${route} is published`,response.status()===200,response.status());}
 for(const route of ['/chords/c-flat-major','/chords/finder']){const response=await request.request.get(base+route);check(`${route} is published`,response.status()===200,response.status());}const progression=await request.request.get(base+'/chord-progressions');check('/chord-progressions is published',progression.status()===200,progression.status());await request.close();
