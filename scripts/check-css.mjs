...
try {
  ...
  await writeFile(join(out,'tailwind-compile.json'),JSON.stringify({executed_at:new Date().toISOString(),passed:true,css_bytes:Buffer.byteLength(compiled),semantic_utilities:candidates.split(' '),declaration_checks:expected.size,cn_checks:5 + sizes.length * 3,authorized_routes:17},null,2)+'\n');
  console.log('Tailwind v4 compilation + semantic utility + cn checks passed for the 17 authorized routes.');
} catch(e) {
