...
for (const url of routes) {
  ...
  const expectedMetadata = editorialMetadata({ title: metadataInputs[url]?.title || source.metadata?.title, description: metadataInputs[url]?.description || source.metadata?.description, alternates: { canonical: url } });
  const sourceTitle = expectedMetadata.title || null;
  const sourceDescription = expectedMetadata.description || null;
