...
try{
  ...
  const chordNav=audit.locator('.site-nav-group').filter({has:audit.locator('.site-nav-parent-link[href="/chords"]')});await chordNav.locator('.site-nav-parent-link').focus();check('Navigation retains Seventh and Add with completion families',same(await chordNav.locator('.site-nav-third-panel[data-kind="more"] .site-nav-third-link').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('href'))),['/chords/seventh','/chords/diminished','/chords/augmented','/chords/suspended','/chords/add','/chords/extended','/chords/altered']));
