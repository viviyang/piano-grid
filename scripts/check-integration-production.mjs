...
import { editorialMetadata } from '../src/lib/seo-editorial.ts';
...
try{
  for(const route of routes){
    ...
    const expectedMetadata=editorialMetadata({title:metadataTitleInputs[route]||source.metadata.title,description:source.metadata.description,alternates:{canonical:route}});
    check(`${route} production title`,await page.title()===expectedMetadata.title);
  ...
  for(const route of ['/','/tools'])for(const width of [1440,390,320,768]){
    ...
    if(width===1440||width===390){
      if(route==='/'){
        ...
        check(`${route} production illustrations ${width}`,await page.locator('.ph-task-illustration svg').count()===7&&await page.locator('.ph-collection-art,.ph-reading-art').count()===2&&await page.locator('.ph-grand-illustration svg').count()===1);
      }else{
  ...
  check(`production sitemap exactly matches ${PUBLIC_ROUTES.length} registered public routes`,JSON.stringify([...sitemapPaths].sort())===JSON.stringify([...PUBLIC_ROUTES].sort())&&!sitemapText.includes('localhost'),sitemapPaths.length);
