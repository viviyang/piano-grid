...
try {
  ...
  const pageCSS = await read('src/app/chords/a-minor/a-minor.css') + await read('src/components/chords/shared.css') + await read('src/components/chords/center.css') + await read('src/components/chords/page-toc.css') + await read('src/components/chords/chord-learning.css') + await read('src/components/keyboard-notes/keyboard-notes.css') + await read('src/components/scales/scales.css') + await read('src/components/songs/songs.css') + await read('src/components/guides/guides.css') + await read('src/components/support/support-pages.css') + await read('src/components/blank-sheet/blank-sheet.css') + await read('src/components/integration/integration.css') + await read('src/components/ui/breadcrumb.css') + await read('src/components/ui/site-brand.css');
  ...
  allowedComponents.push(...['site-chrome','playback-controls','detail-page','keyboard-viewport','print-voicing','center-page','center-experience','page-toc','fingering-guide','chord-builder-practice'].map(n=>`src/components/chords/${n}.tsx`));
  allowedComponents.push('src/components/chords/shared.css','src/components/chords/center.css','src/components/chords/page-toc.css','src/components/chords/chord-learning.css');
  ...
  const allowedPages=['src/app/page.tsx','src/app/tools/page.tsx','src/app/chords/page.tsx',...['a-minor','a-major','c-major','g-major','c-minor','e-major','b-major','a-flat-major','c-flat-major','by-key','finder'].map(n=>`src/app/chords/${n}/page.tsx`)];
  ...
  check('Only twenty-eight authorized routes', JSON.stringify(srcFiles.filter(p => /\/(page|route|not-found)\.[mc]?[jt]sx?$/.test(p)).sort()) === JSON.stringify([...allowedPages].sort()));
  ...
  check('Only authorized static assets', publicFiles.every(p => ['public/favicon.ico','public/favicon.svg','public/favicon-96x96.png','public/apple-touch-icon.png','public/icon-192.png','public/icon-512.png','public/assets/home/east-lake-grand-piano.webp','public/assets/home/east-lake-piano-hero.png','public/assets/home/pianogrid-hero-sunset.png','public/assets/chords/a-minor-notes-inversions.pdf','public/assets/guides/piano-starter-and-reading.pdf','public/downloads/scales/pianogrid-scales-notes-check-worksheet.pdf','public/downloads/scales/pianogrid-scales-starter-reference.pdf','public/reference/assets/chord-a-major.pdf','public/reference/assets/chord-c-major.pdf',...["g-major","c-minor","e-major","b-major","a-flat-major","c-flat-major"].flatMap(slug=>[`public/reference/assets/chord-${slug}.pdf`,`public/reference/assets/chord-${slug}.svg`]),'public/reference/assets/blank-piano-staff-letter.pdf','public/reference/assets/blank-piano-staff-a4.pdf','public/reference/assets/blank-piano-staff-preview.svg','public/reference/preserved-chords/assets/piano-chord-chart-selected.pdf', ...[88,61].flatMap(n=>['octaves','letters'].map(m=>`public/reference/generated/keyboard-notes/labeled-${n}-${m}.pdf`))].includes(p)));
