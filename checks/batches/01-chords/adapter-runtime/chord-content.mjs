import { readAuthorizedPage, localPreview, authorizedURLs } from './site-content.mjs';
...
export function getChordDetail(url) {
    ...
    const related = d.related.filter((u) => localPreview && authorizedURLs.includes(u));
    if (related.length)
        blocks.push({ block_id: `${prefix}-related`, content: { ...empty('Related local previews'), links: related.map((u) => ({ url: u, label: u === '/chords' ? 'Piano chord chart' : u, published: true })) } });
...
export function getChordCenter() {
    ...
    const items = page.data.chord_ids.map((id) => { const c = shared.chords[id], v = shared.voicings[c.root_voicing_id]; if (!v)
        throw new Error(`Missing ${id}`); return { id, tones: c.note_spellings, formula: c.formula_degrees, name: c.name_en, root: c.root_spelling, quality: c.quality, url: localPreview && authorizedURLs.includes(c.canonical_url) ? c.canonical_url : null, voicing: { voicing_id: v.voicing_id, inversion_label: v.inversion_label, chord_symbol: v.chord_symbol, bass_spelling: v.bass_spelling, notes_low_to_high: v.notes_low_to_high.map((n) => ({ display_pitch: n.display_pitch, midi: n.midi })), diagram: v.diagram, playback: { together: v.playback.together, ascending: v.playback.ascending }, print_data: v.print_data } }; });
    const comparisons = Object.values(shared.comparisons).map(pair => { const p = pair; return { left: items.find(i => i.voicing.voicing_id === p.left), right: items.find(i => i.voicing.voicing_id === p.right) }; });
