...
try{
  for(const width of [1440,390]){
    for(const kind of ['reference','page']){
      ...
      await p.screenshot({path:`checks/batches/01-chords/${kind}-${width}-top.png`});
      await p.screenshot({path:`checks/batches/01-chords/${kind}-${width}-full.png`,fullPage:true});
      await writeFile(`checks/batches/01-chords/${kind}-${width}-snapshot.txt`,await p.locator('body').ariaSnapshot());
  ...
  await writeFile('checks/batches/01-chords/visual-comparison.json',JSON.stringify(results,null,2));
