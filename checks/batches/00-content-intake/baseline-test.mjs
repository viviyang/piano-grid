...
const out=resolve('checks/batches/00-content-intake');await mkdir(out,{recursive:true});
...
try{
  ...
  check('Noindex retained',(await page.locator('meta[name="robots"]').getAttribute('content')).includes('noindex'));
  check('No fabricated canonical',await page.locator('link[rel="canonical"]').count()===0);
  ...
  for(const b of content.blocks){
    if(b.block_id==='am-next'){check('am-next hidden only because targets unpublished',b.content.links.every(l=>!l.published)&&await page.locator('[data-block-id="am-next"]').count()===0);continue;}
  ...
  check('No dead page links',links.every(h=>h.startsWith('#')||h==='/assets/chords/a-minor-notes-inversions.pdf'),links);
  check('No public fingering controls',await page.locator('input[name*="finger"],select[name*="finger"]').count()===0);
  ...
  for(const scenario of ['unavailable','error','delayed-stop','delayed-selection']){
    ...
    if(scenario==='unavailable')check('Unavailable audio is disabled and explained',await p.getByRole('button',{name:'Play chord',exact:true}).isDisabled()&&await p.locator('#audio-status').textContent()===content.microcopy.audio_unavailable);
      ...
      if(scenario==='error')check('Startup error preserves correct answer',await p.locator('#audio-status').textContent()===content.microcopy.audio_error&&await p.locator('#current-symbol').textContent()==='Am');
        ...
        check(`${scenario}: late resume schedules no sound`,await p.evaluate(()=>window.__audio.nodes.length===0));
      }
    }
  ...
  check('Root stays unimplemented',(await page.request.get(`${base}/`)).status()===404);
