...
const root="C:/Users/Admin/Documents/viviyang_github/piano";
const results = [];
const warnings = [];
...
try {
  ...
  const pageCSS = await read('src/app/chords/a-minor/a-minor.css') + await read('src/components/chords/shared.css') + await read('src/components/chords/center.css') + await read('src/components/keyboard-notes/keyboard-notes.css');
  ...
  check('Only seven authorized routes', JSON.stringify(srcFiles.filter(p => /\/(page|route|not-found)\.[mc]?[jt]sx?$/.test(p)).sort()) === JSON.stringify([...allowedPages].sort()));
  ...
  check('Only authorized static PDF', publicFiles.every(p => ['public/assets/chords/a-minor-notes-inversions.pdf','public/reference/assets/chord-a-major.pdf','public/reference/assets/chord-c-major.pdf','public/reference/preserved-chords/assets/piano-chord-chart-selected.pdf', ...[88,61].flatMap(n=>['octaves','letters'].map(m=>`public/reference/generated/keyboard-notes/labeled-${n}-${m}.pdf`))].includes(p)));
  ...
  report.stage = '02-keyboard-notes';
  await mkdir(join(root,'checks/reviews/02-keyboard-notes/machine/foundation'),{recursive:true});
  await writeFile(join(root,'checks/reviews/02-keyboard-notes/machine/foundation/foundation.json'),JSON.stringify(report,null,2)+'\n');
