...
r=Path.cwd();e=r/'checks/product-system-p1-2026-09-22';d=r/'docs/product-system-p1-2026-09-22'
def read(p):return json.loads(p.read_text(encoding='utf-8-sig'))
def write(p,x):p.write_text(json.dumps(x,ensure_ascii=False,indent=2),encoding='utf8')
# Copy only explicit paths returned by our successful browser calls.
for receipt,name in [('browser-receivers-supported-receipt.json','browser-receivers.json'),('browser-receivers-corrected-receipt.json','browser-receivers-corrected.json'),('browser-nine-receipt.json','browser-nine.json'),('browser-music-final-receipt.json','browser-music-final.json')]:
 v=read(e/receipt)['result']['value'];(e/name).write_bytes(Path(v['path']).read_bytes())
write(e/'browser-final-smoke.json',read(e/'browser-smoke-read-receipt.json')['result']['value']['smoke'])
base=read(e/'inherited-file-hashes.json');current={str(p.relative_to(r)).replace('\\','/'):hashlib.sha256(p.read_bytes()).hexdigest() for folder in ['src','scripts'] for p in (r/folder).rglob('*') if p.is_file()};changed=[p for p,h in current.items() if base.get(p)!=h];new=[p for p in changed if p not in base];write(e/'p1-change-scope.json',{'new':new,'modifiedFromP0':[p for p in changed if p in base],'hashes':{p:current[p] for p in changed}})
meta=read(e/'metadata-comparison.json');bundle=read(e/'bundle-size.json')['builds'];delta=bundle['.next-p1-final']['allChunkBytes']-bundle['.next-p0-final']['allChunkBytes']
protected=subprocess.check_output(['git','diff','--name-only','--','public','docs/content','package.json','package-lock.json','src/app','src/lib/a-minor-audio.ts','src/components/scales/use-scale-audio.ts','src/components/scales/scale-learning.tsx'],text=True).splitlines();audit=read(r/'checks/product-system-p0-2026-09-22/audit-materials-manifest.json');auditChanges=[x['path'] for x in audit if hashlib.sha256((r/x['path']).read_bytes()).hexdigest()!=x['sha256']]
write(e/'final-state.json',{'time':datetime.datetime.now(datetime.timezone.utc).isoformat(),'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'originMainFinalFetch':subprocess.check_output(['git','rev-parse','origin/main'],text=True).strip(),'branch':'codex/product-system-p1','scope':'P1','status':'BLOCKED','protectedDiff':protected,'auditChanged':auditChanges,'lockSHA256':hashlib.sha256((r/'package-lock.json').read_bytes()).hexdigest()})
rows='\n'.join(f'| `{p}` | '+('新增' if p in new else 'P0之后修改')+' |' for p in changed)
report=f'''# BLOCKED，scope=P1
...
审计51份文件hash变化数={len(auditChanges)}；原P0 RESULT保留历史结论。受保护内容、public资产、package/lock、src/app与既有音乐引擎diff={protected}。没有新增SEO路由、索引策略、TDH、导航、主题或依赖。采用组件规则快照2026-09-21，已有普通anchor/select/radio与am-button样式足够，不引入新控件库。
...
| 性能 | 无新SDK/库 | 总输出JS chunks +{delta} bytes（约{delta/bundle['.next-p0-final']['allChunkBytes']*100:.2f}%）；这不是单页下载量或CWV | bundle-size.json；Lighthouse/真机性能NOT_RUN |
...
{rows}
'''
(d/'RESULT.md').write_text(report,encoding='utf8')
(e/'exits.txt').write_text('state_tests=0\nadapter_tests=0\ncheck_final=0\ndata_final=0\nscale_final=0\np0_measurement_regression=0\nhash_regression=0\nbuild=0\nbuild_final=0\ngit_diff_check=0\npdf_text_equality=8/8\npdf_visual=FAIL\nbrowser_screenshot=FAIL\n',encoding='utf8')
print('P1 RESULT and evidence index written; status BLOCKED')
